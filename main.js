...
var c=[]
async function demo(){
    const data= await fetch("https://api.rootnet.in/covid19-in/unofficial/covid19india.org/statewise")
    const dd= await data.json()
    const sdwd=await fetch('https://api.covid19india.org/state_district_wise.json')
    ...
    if(dl==="Maharashtra")
    {
      ddo=Object.values(ddata[dl])
        ...
        var nnd=["Unknown", "Railway Quarantine", "Airport Quarantine", "Other State","Foreign Evacuees","Bengaluru Rural","Bengaluru Urban"]
        nnd.forEach(i=>{
          if(c.includes(i))
          {
           removeItemOnce(c,i)
         ...
         c.forEach((i)=>{
           ...
           var dn=document.querySelector('#t3').value
            document.querySelector('.dconfirm').textContent=ddata[dl].districtData[dn].confirmed
            document.querySelector('.dactive').textContent=ddata[dl].districtData[dn].active
            document.querySelector('.drecover').textContent=ddata[dl].districtData[dn].recovered
            document.querySelector('.ddeath').textContent=ddata[dl].districtData[dn].deceased
        }
...
async function dwd(){
        ...
        nnd.forEach(i=>{
         ...
         c.forEach((i)=>{
           ...
           if(c[0]===zz){
            document.querySelector('.dconfirm').textContent=ddata[sd].districtData[zz].confirmed
            document.querySelector('.dactive').textContent=ddata[sd].districtData[zz].active
            document.querySelector('.drecover').textContent=ddata[sd].districtData[zz].recovered
            document.querySelector('.ddeath').textContent=ddata[sd].districtData[zz].deceased
           }
